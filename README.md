...
![alt text](https://github.com/rheiland/xml2jupyter/blob/master/images/biorobots_microenv_med.png)

![alt text](https://github.com/rheiland/xml2jupyter/blob/master/images/biorobots_user_params_med.png)
...
The `xml2jupyter.py` script generates two panels of widgets that represent custom parameters for a PhysiCell simulation. But our ultimate goal is to provide a full-featured GUI for a PhysiCell simulation. By providing additional, static Python modules to accompany the dynamically-generated custom parameters modules (from `xml2jupyter.py`), we can achieve this.
...
![model description](images/mygui_about.png)
...
![custom user parameters](images/mygui_microenv.png)

![custom user parameters](images/mygui_user_params.png)


and the `Cell Plots` and `Substrate Plots` which will display pre-computed data when you click the `frame` slider:

![cells](images/mygui_cells.png)

![substrate fields](images/mygui_oxygen.png)

The following images show a Jupyter notebook GUI for the same model, with additional substrate plots, running on [nanoHUB](https://nanohub.org/) (showing v1 of xml2jupyter, prior to adding the `Microenvironment` tab).
